...
public class ShowVideoAction extends AbstractAction {
	private final JTable table;
	private Model Model = null;

	public ShowVideoAction(JTable table,Model model,String ButtonName) {
		super(ButtonName);
		this.table = table;
		Model = model;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(model != null){
			/*Desktop.getDesktop().open(new Fil);
			Desktop.getDesktop().open(new File("test.mp4"));
		    ((Model)model).getVideo().getDataInStream()*/
